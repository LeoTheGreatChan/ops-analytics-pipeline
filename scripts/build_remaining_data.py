"""
KPIs, segments, and forecast JSON builder.

Refactored (Render-deployment fix) into a callable function so app.py can
call it in-process instead of via subprocess, same reasoning as the other
two refactored scripts: avoids redundant pandas loads competing for memory
...
def build_remaining_data(cleaned_data_path='../data/processed/cleaned_data.pkl',
    ...
    with open(f'{output_dir}/forecast.json', 'w') as f:
        json.dump(forecast, f, indent=2)

    if verbose:
        print("KPIs:", json.dumps(kpis, indent=2))

    return kpis, segments, forecast


if __name__ == '__main__':
    kpis, segments, forecast = build_remaining_data()
    print()
